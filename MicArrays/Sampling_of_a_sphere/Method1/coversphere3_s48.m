...
% 160822PJ: slightly adjusted cable duct position
...
iTh = [0:5:360];
aColour = [0.8 0.1 0.1];% hemisphere join colour
rColourx = [0.0 0.0 1.0 ];% microphone colour
rColourg = [0.5 0.5 0.95 ];% grommet colour
rColourf = [0.5 0.95 0.5 ];% fixing colour
hAz = 9;% -5.625 deg, half of 11.25 deg
hEl = 0;% 2.5, 7.5 deg, half of 15 deg
cEl = -58;% cable duct at the rear
% cAz = 180;% cable duct at the rear, just beside the microphone stand
cAz = 180 - (360 * 0.025 / (2 * pi * r2 * cos(cEl*pi/180)));
fAz = 180 * [1; 1];
fEl = [(12.6 - 60.0); (-13.2 - 60.0)];
nF = 2;

a = load('m48sphere.txt');
[nM,nC] = size(a);
mAz = a(:,2)-a(4,2);% 4 works, 9 almost works
mEl = a(:,3);
...
set(hs,'Facecolor',[0.99 0.925 0.75]);
l = light;
lighting phong;
hold on;
